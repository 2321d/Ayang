...
$sql = "UPDATE pesanan 
        SET nama = ?, email = ?, nomer = ?, jenis_kopi = ?, ukuran = ?, toping_tambahan = ?, jumlah = ?, alamat = ?, tanggal_pesan = ?, total_harga = ?
        WHERE id = ?";
$stmt = $conn->prepare($sql);
if ($stmt === false) {
    die("Prepare statement gagal: " . $conn->error);
}
$stmt->bind_param('sssssssssii', $nama, $email, $nomer, $jenis_kopi, $ukuran, $toping_tambahan, $jumlah, $alamat, $tanggal_pesan, $total_harga, $id);
